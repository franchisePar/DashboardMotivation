...
function App() {
  ...
  // ── FIX: Today count ──
  // Option 1: Si le backend envoie confirmedBookings dans todayStats
  const todayCount = stats.confirmedBookings || stats.confirmed || 0
  
  // Option 2: Si tu dois filtrer depuis latestReservations (décommente si Option 1 ne marche pas)
  /*
  const todayCount = useMemo(() => {
    const all = data?.latestReservations || []
    const confirmed = all.filter(b => {
      const status = (b.status || '').toString().toUpperCase().trim()
