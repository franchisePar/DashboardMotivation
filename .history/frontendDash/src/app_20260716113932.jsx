...
function App() {
  ...
  // ── FIX: Use confirmed-only counts from backend ──
  ...
  // ── FIX: Today count = confirmed only (from backend) ──
  const todayCount = stats.totalBookings || 0

  // ── FIX: Use backend data directly for charts ──
  const countryStats = data?.todayCountryStats || []  // ← FIX: was countryStats (all-time)
  const hourlyData = data?.hourlyData || []
  const activeCountries = countryStats.length

  // ── FIX: Clean "1h - 3la" text from bookings for display ──
  ...
  // ── Map: confirmed only for dot highlighting ──
        ...
        <main className="app-main">
          <section className="kpi-row">
            ...
            <StatCard
              ...
              value={totalThisMonth}  // ← FIX: was totalFromMeta (all time)
              icon={<CalendarCheck size={40} />}
              color="#22c55e"
              trend="up"
          ...
          <section className="bottom-section">
            <CountryLeaderboard countries={countryStats} />  {/* ← FIX: uses todayCountryStats */}
