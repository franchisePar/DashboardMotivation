...
<StatCard
  ...
  value={(() => {
    ...
    const top = entries.sort((a, b) => (b[1].todayBookings || 0) - (a[1].todayBookings || 0))[0]
    return top ? top[0] : '—'
  })()}
  icon={<img src={brandLogo((() => {
    const entries = Object.entries(brandStats)
    const top = entries.sort((a, b) => (b[1].todayBookings || 0) - (a[1].todayBookings || 0))[0]
    return top ? top[0] : 'UNITED'
  })())} alt="Top Brand" style={{ height: '32px', width: 'auto' }} />}
  color="#f59e0b"
  sub={(() => {
    const entries = Object.entries(brandStats)
    const top = entries.sort((a, b) => (b[1].todayBookings || 0) - (a[1].todayBookings || 0))[0]
    return top ? `${top[1].todayBookings} bookings` : '0 bookings'
  })()}
