...
const BRAND_CONFIG = {
  UNITED: { color: '#0f27a2', bgSoft: '#e8ecf8' },
  MOVIS:  { color: '#f94231', bgSoft: '#fdecea' },
  DRIVO:  { color: '#a3c520', bgSoft: '#f3f7e0' },
}

export function BrandStats({ stats }) {
  const brandData = useMemo(() => {
    return Object.entries(stats).map(([brand, data]) => {
      const config = BRAND_CONFIG[brand] || { color: '#64748b', bgSoft: '#f1f5f9' }
      
      const today = data?.todayBookings ?? data?.today ?? 0
      const yesterday = data?.yesterdayBookings ?? data?.yesterday ?? 0
  ...
  // If all brands show 0, it's a data issue — show a subtle hint
