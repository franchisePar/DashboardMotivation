...
// ── Glow marker with pulse animation ──
function createGlowMarker(color) {
  return L.divIcon({
    className: 'glow-marker',
    html: `
      <div class="glow-ring" style="--glow-color: ${color}"></div>
      <div class="glow-core" style="--glow-color: ${color}"></div>
    `,
    iconSize: [24, 24],
    iconAnchor: [12, 12],
    popupAnchor: [0, -12],
...
export function LiveMap({ bookings = [] }) {
  ...
  useEffect(() => {
    ...
    // ═══════════════════════════════════════════════════════
    // STYLE VOYAGER — clair, pro, match ton dashboard
    // ═══════════════════════════════════════════════════════
  ...
  useEffect(() => {
    ...
    markersToShow.forEach((marker) => {
      const m = L.marker([marker.lat, marker.lng], {
        icon: createGlowMarker(marker.color),
      })
