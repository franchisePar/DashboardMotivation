...
// const MAP_STYLE = 'https://openmaptiles.github.io/dark-matter-gl-style/style-cdn.json'
...
// Brand colors: neon glow for dark backgrounds
const BRAND_COLORS = {
  UNITED: { 
    fill: '#0f27a2', 
    stroke: '#4a7bff', 
  ...
  MOVIS:  { 
    fill: '#f94231', 
    stroke: '#ff7b6b', 
    glow: 'rgba(255,123,107,0.6)',
    neon: '#ff7b6b'
  },
  DRIVO:  { 
    fill: '#a3c520', 
    stroke: '#d4f060', 
    glow: 'rgba(212,240,96,0.6)',
    neon: '#d4f060'
  },
}
...
// Create upgraded neon marker for dark theme
...
export function LiveMap({ bookings = [] }) {
  ...
  useEffect(() => {
    ...
    // Add canvas overlay for neon arcs
        ...
        .setPopup(
          new maplibregl.Popup({ offset: 10 }).setHTML(`
            <div style="padding:10px 14px;font-family:system-ui,sans-serif;background:#1a1a2e;color:#fff;border-radius:8px;">
              <div style="font-weight:700;font-size:14px;margin-bottom:6px;color:${BRAND_COLORS[brand]?.neon || '#fff'};">
                ${d.country}
              </div>
              <div style="font-size:12px;color:#aaa;margin-bottom:4px;">
                ${d.count} reservations
              </div>
              ${Object.entries(d.brands).map(([b, c]) => `
                <div style="font-size:11px;color:${BRAND_COLORS[b]?.neon || '#888'};">
                  ● ${b}: ${c}
                </div>
              `).join('')}
            </div>
  ...
  const styles = [
    ...
    { name: 'Alidade Dark', url: 'https://tiles.stadiamaps.com/styles/alidade_smooth_dark.json' },
