...
export function LiveMap({ bookings = [], theme }) {
  ...
  // Circular star field around the Earth.
  // The alpha texture makes each Three.js point a real round star instead
  // of the square sprite produced by the default PointsMaterial.
  useEffect(() => {
    ...
    for (let i = 0; i < starCount; i += 1) {
      const radius = 155 + Math.random() * 105
    ...
    // Small circular alpha mask for round stars.
    ...
    const material = new THREE.PointsMaterial({
      ...
      transparent: true,
      opacity: currentTheme === 'dark' ? 0.82 : 0.10,
      map: starTexture,
      alphaTest: 0.02,
      depthWrite: false,
      depthTest: true,
  ...
  // Configure the camera ONLY when the globe is created.
  // IMPORTANT: do not depend on globeSize here. The sidebar can resize the map,
  // and changing the size must never reset a user's current Earth position.
  const handleGlobeReady = () => {
    const globe = globeRef.current
    if (!globe || cameraInitializedRef.current) return
    ...
    // Initial view: Morocco / Europe centered in the map card, with the Earth zoomed in.
    // This runs once only — refresh/remount starts here again, but sidebar
    // hover/resizing will NOT move the Earth back to this position.
    globe.pointOfView(
      {
        lat: HUB.lat,
        lng: HUB.lng,
  ...
  // The Morocco hub must show Morocco's reservation count,
  // NOT bookings.length (which is the total across every country).
  const moroccoReservationCount = useMemo(() => {
  ...
  }, [visibleCountries, moroccoReservationCount])
  ...
  return (
    ...
    >
      <Globe
        ...
        // Hover labels
        labelsData={points.filter(d => d.count > 0)}
        labelLat={d => d.lat}
        labelLng={d => d.lng}
        labelText={d =>
          `${d.name === 'MOROCCO HUB' ? 'MOROCCO' : d.name} · ${d.count} reservation${d.count === 1 ? '' : 's'}`
        }
        ...
        onPointClick={d => {
          if (globeRef.current) {
            globeRef.current.pointOfView(
              { lat: d.lat, lng: d.lng, altitude: 1.55 },
              900
            )
          }
        }}
      ...
      <style>{`
        /* WebGL layer stays behind all dashboard controls. */
        ...
        /* UI overlays must remain visible even when the Earth is zoomed in. */
        .live-map-3d > div:not(:first-child) {
          z-index: 50 !important;
        }

        /* Match the dashboard reference: the Earth sits slightly lower
           and large enough to be cropped naturally by the map card. */
