...
export function NotificationToast({ booking, onDismiss }) {
  ...
  // ═══════════════════════════════════════════════════════════
  // 🔥 CRITICAL DEBUG — Log everything we received
  // ═══════════════════════════════════════════════════════════
  console.log('═══════════════════════════════════════')
  console.log('🔔 NOTIFICATION RECEIVED')
  console.log('═══════════════════════════════════════')
  console.log('booking object:', booking)
  console.log('booking?.brand:', booking?.brand)
  console.log('booking?.status:', booking?.status)
  console.log('booking?.country:', booking?.country)
  console.log('booking?.reservationNumber:', booking?.reservationNumber)
  console.log('All keys:', Object.keys(booking || {}))
  console.log('═══════════════════════════════════════')

  // ── Parse & FIX data ────────────────────────────────────────
  
  // BRAND: Get from booking.brand, fallback to UNITED
  let currentBrand = (booking?.brand || 'UNITED').toString().toUpperCase().trim()
  
  // STATUS: Get from booking.status
  let rawStatus = (booking?.status || 'CONFIRMED').toString().toUpperCase().trim()
  
  // 🛡️ BUG DETECTION: If status contains a brand name, the data is corrupted
  if (BRAND_NAMES.includes(rawStatus)) {
    console.error('🐛 BUG: booking.status contains BRAND NAME:', rawStatus)
    console.error('   This means status field has wrong data!')
    
    // Try to find real status in other fields
    const possibleFields = ['reservationStatus', 'state', 'bookingStatus', 'orderStatus', 'finalStatus']
    ...
    for (const field of possibleFields) {
      ...
      if (val && typeof val === 'string') {
        ...
        if (!BRAND_NAMES.includes(upperVal)) {
          if (upperVal.includes('CONFIRM') || upperVal.includes('CANCEL')) {
            foundStatus = upperVal
            console.log('✅ Found real status in field "' + field + '":', foundStatus)
            break
          }
        }
      }
    ...
    // If still not found, scan ALL fields
    if (!foundStatus) {
      for (const [key, val] of Object.entries(booking || {})) {
        if (typeof val === 'string') {
          const upperVal = val.toUpperCase().trim()
          if (upperVal === 'CONFIRMED' || upperVal === 'CONFIRMED' || upperVal === 'CANCELED' || upperVal === 'CANCELLED') {
            foundStatus = upperVal
            console.log('✅ Found real status in field "' + key + '":', foundStatus)
            break
          }
        }
      }
    ...
    rawStatus = foundStatus || 'CONFIRMED'
    console.log('🔄 Corrected status to:', rawStatus)
  }
  
  // 🛡️ BUG DETECTION: If brand is always UNITED, check if real brand is in status or other fields
  if (currentBrand === 'UNITED' && booking?.status) {
    const statusUpper = booking.status.toString().toUpperCase().trim()
    if (['MOVIS', 'DRIVO'].includes(statusUpper)) {
      console.error('🐛 BUG: booking.status contains BRAND instead of status:', statusUpper)
      // Status field has brand, try to find real brand elsewhere
      // Keep UNITED as default or try to detect from other fields
    }
  }
  ...
  console.log('   Brand color:', brandConfig.color)
  console.log('   Status color:', statusConfig.text)
    ...
    <>
      ...
      >
        ...
        <div style={{
          ...
          {/* STATUS — Uses CORRECTED rawStatus */}
          <div style={{
            display: 'flex',
            alignItems: 'center',
            gap: '10px',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              ...
              {/* ✅ USE rawStatus (corrected), NOT booking?.status */}
              <span style={{ 
                fontSize: '15px', 
