...
export function RecentBookings({ bookings: incomingBookings }) {
  ...
  // Handle incoming bookings - can be array (from dashboard_update) or single (from new_booking)
  useEffect(() => {
    if (!incomingBookings) return

    // Normalize to array
    const newBookings = Array.isArray(incomingBookings) ? incomingBookings : [incomingBookings]

    if (newBookings.length > 0) {
      setAllBookings(prev => {
        // If incoming is the full latestReservations array (30 items), replace entirely
        if (newBookings.length >= 10) {
          return newBookings.slice(0, MAX_BOOKINGS)
        }

        // Otherwise merge with existing
        const merged = [...newBookings, ...prev]
        const seen = new Set()
        const unique = merged.filter(b => {
          const key = `${b.reservationNumber || b.id}-${b.receivedAt || b.receivedAtDate}`
