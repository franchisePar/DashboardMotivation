...
// ═══════════════════════════════════════════════════════════
// STYLE: Minimal grey — clean background for 3D globe
// ═══════════════════════════════════════════════════════════
...
export function LiveMap({ bookings = [] }) {
  ...
  const layers = useMemo(() => {
    ...
    return [
      ...
      // ScatterplotLayer: dots on globe surface
      new ScatterplotLayer({
        ...
        getRadius: (d) => Math.sqrt(d.count) * 10000,
        getLineColor: (d, { index }) => colors[index].stroke,
        stroked: true,
        lineWidthMinPixels: 1.5,
        lineWidthMaxPixels: 3,
        opacity: 0.9,
        // 3D effect: dots stick to globe surface
        billboard: false,
      }),
    ]
  }, [locationData])
  ...
  // Initialize map with GLOBE view
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      zoom: 1.4,               // Wide zoom for globe feel
      pitch: 60,               // Tilted for 3D perspective
      bearing: 0,
      attributionControl: false,
      // Enable projection for globe-like feel
      projection: 'globe',     // MapLibre 2+ globe projection
    })
    ...
    mapContainerRef.current.addEventListener('mouseenter', handleMouseEnter)
    mapContainerRef.current.addEventListener('mouseleave', handleMouseLeave)

    return () => {
      if (rotationRef.current) cancelAnimationFrame(rotationRef.current)
      mapContainerRef.current?.removeEventListener('mouseenter', handleMouseEnter)
  ...
  return (
    <div
      ...
      style={{
        ...
        height: '100%',
        borderRadius: '14px',
        background: '#f0f0f0', // Grey background behind map
      }}
    />
  )
}