...
export function LiveMap({ bookings = [] }) {
  ...
  const [layerMode, setLayerMode] = useState('columns')
  ...
  // Initialize MapLibre in its own container
  useEffect(() => {
  ...
  // Initialize DeckGL in a separate overlay container
  useEffect(() => {
  ...
  // Sync MapLibre movement to DeckGL
  useEffect(() => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', borderRadius: '14px', overflow: 'hidden' }}>
      ...
      {/* MapLibre container (bottom layer) */}
      <div
        ref={mapContainerRef}
        style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%' }}
      />

      {/* DeckGL overlay container (top layer, transparent) */}
      <div
        ...
        style={{
          ...
          width: '100%',
          height: '100%',
          pointerEvents: 'none', // Let clicks pass through to map when not on deck elements
        }}
      />
    </div>
  )
