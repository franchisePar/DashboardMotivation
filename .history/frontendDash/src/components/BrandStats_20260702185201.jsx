...
export function BrandStats({ stats }) {
  ...
  // Use todayBookings for the bar chart
  const maxToday = Math.max(...brands.map(b => safeStats[b]?.todayBookings || 0), 1)
    ...
    <div className="brand-stats">
      ...
      <div className="brand-stats__list">
        {brands.map((brand) => {
          ...
          const width = `${(s.todayBookings / maxToday) * 100}%`  // ← TODAY
            ...
            <div key={brand} className="brand-stats__row">
              <div className="brand-stats__meta">
                ...
                <div className="brand-stats__info">
                  <span className="brand-stats__total">
                    {formatNumber(s.todayBookings)} today  {/* ← TODAY */}
                  </span>
                </div>
              </div>
              ...
              <div className="brand-stats__bar-row">
                ...
                <span className="brand-stats__val" style={{ color }}>
                  {formatNumber(s.todayBookings)}  {/* ← TODAY */}
                </span>
              </div>
            </div>
          )
