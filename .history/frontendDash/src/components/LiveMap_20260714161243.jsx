...
// ═══════════════════════════════════════════════════════════
// NATURAL EARTH / "DODO" STYLE — Free options
// ═══════════════════════════════════════════════════════════
// Natural Earth (terrain-like, soft colors)
const MAP_STYLE = 'https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json'

// Alternative: Natural Earth with terrain
// const MAP_STYLE = 'https://demotiles.maplibre.org/style.json'

// Alternative: Stadia Terrain (free tier)
...
// ALL partner countries — dots show on ALL of them
...
const HUB = [10, 35] // Central hub (Mediterranean)
...
export function LiveMap({ bookings = [] }) {
  ...
  // Build dot data: ALL partner countries + booking counts if available
  const dotData = useMemo(() => {
    // Start with all partner countries
    const dots = ALL_PARTNER_COUNTRIES.map(c => ({
      position: [c.lng, c.lat],
      country: c.name,
      brand: c.brand,
      count: 0, // Will be updated if bookings exist
      hasBookings: false,
    }))
    ...
    bookings.forEach((booking) => {
      ...
      if (dot) {
        ...
        // Override brand if booking has different brand
  ...
  // Build arc data: from each country to hub, colored by country's brand
  ...
  const layers = useMemo(() => [
    // Arcs: from each country to hub, colored by brand
    new ArcLayer({
      id: 'arcs',
      data: arcData,
      pickable: true,
    ...
    // Dots: on ALL partner countries, colored by brand
    // Countries WITH bookings get bigger dots
    new ScatterplotLayer({
      id: 'dots',
      data: dotData,
      pickable: true,
      radiusScale: 40,
      radiusMinPixels: 5,     // Small for no bookings
      radiusMaxPixels: 16,    // Bigger for has bookings
      getPosition: d => d.position,
      getFillColor: d => {
        const c = BRAND_COLORS[d.brand]?.dot || [15, 39, 162]
        return d.hasBookings ? [...c, 230] : [...c, 120] // More opaque if has bookings
      },
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      ...
      style: MAP_STYLE,
      center: [10, 20],
      zoom: 1.2,          // Zoomed out to see full globe
      pitch: 45,           // 3D tilt for globe feel
      bearing: 0,
    ...
    // Add atmosphere glow for "real earth" feel
    map.once('load', () => {
      // Add custom atmosphere if supported
