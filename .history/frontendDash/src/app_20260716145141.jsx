...
function App() {
  ...
  // ── Brand stats mapping — ROBUST VERSION ──
  const brandStats = useMemo(() => {
    ...
    // Handle case where backendBrandStats might be array or have weird structure
    let entries = [];
    if (Array.isArray(backendBrandStats)) {
      entries = backendBrandStats.map(item => [item.brand, item]);
    ...
    entries.forEach(([brand, s]) => {
      ...
      console.log(`Processing brand: "${brand}" -> key: "${key}", data:`, JSON.stringify(s));
      ...
      if (counts[key]) {
        ...
        console.log(`  -> Key "${key}" not found in counts!`);
      }
    });
