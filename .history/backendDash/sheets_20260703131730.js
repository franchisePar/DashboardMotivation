...
function parseDate(raw) {
  ...
  // Try 1: Direct ISO string (2026-07-02 14:30:00 or 2026-07-02T14:30:00)
  let d = new Date(str);
  if (!isNaN(d.getTime())) {
  ...
  if (/^\d+\.?\d*$/.test(str)) {
    ...
    d = new Date(excelEpoch.getTime() + ms - 86400000);
    if (!isNaN(d.getTime())) {
  ...
  // Try 7: n8n format "2026-07-03 12:11:00" (space instead of T)
  ...
  // Try 8: European format with dots DD.MM.YYYY HH:MM
  m = str.match(/(\d{1,2})\.(\d{1,2})\.(\d{4})[ T](\d{1,2}):(\d{2})/);
  if (m) {
    const [, day, month, year, hour, minute] = m;
    d = new Date(`${year}-${month.padStart(2,'0')}-${day.padStart(2,'0')}T${hour.padStart(2,'0')}:${minute}:00`);
...
// ═══════════════════════════════════════════════════════════
// isToday — Compare dates properly with timezone fix
// ═══════════════════════════════════════════════════════════
function isToday(date) {
  ...
  console.log('isToday:', date.toISOString(), '===', today.toISOString(), '?', result);
  return result;
}
