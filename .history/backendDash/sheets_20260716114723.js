...
// ═══════════════════════════════════════════════════════════
// isToday — Compare LOCAL dates (fixes 1h UTC offset)
// ═══════════════════════════════════════════════════════════
...
function buildDashboardData(reservations) {
  ...
  // ── All today bookings (all statuses) ──
  ...
  // ── CONFIRMED only this month (for "Reservations This Month") ──
  const now = new Date();
  ...
  // ── Recent bookings: ALL STATUSES (for live feed + notifications) ──
  ...
  return {
    meta: {
      ...
      totalReservations: reservations.length,                    // ALL (for reference)
      totalConfirmedReservations: reservations.filter(r => r.status === 'CONFIRMED').length,
      totalThisMonth: thisMonthConfirmed.length,                  // ← CONFIRMED this month
    },
    todayStats: {
    ...
    brandStats,
    countryStats,        // All-time (for reference if needed)
    todayCountryStats,   // Today's confirmed (for dashboard)
    latestReservations,  // ALL statuses (for live feed)
    leaderboard,
    hourlyData,          // Today confirmed only
  };
}
