...
// ═══════════════════════════════════════════════════════════
// isToday — Compare LOCAL dates (fixes 1h offset)
// ═══════════════════════════════════════════════════════════
function isToday(date) {
  ...
  // Use LOCAL date strings, not UTC (toISOString)
  const dateStr = date.toLocaleDateString('sv-SE'); // "2026-07-16"
  const today = new Date();
  const todayStr = today.toLocaleDateString('sv-SE');

  // Accept today OR yesterday (for testing)
...
async function fetchSheetData() {
  try {
    ...
    console.log('🔍 SERVER TODAY (LOCAL):', new Date().toLocaleDateString('sv-SE'));
    ...
    console.log('═══════════════════════════════════════');
    console.log('TODAY BOOKINGS (all statuses):', todayBookings.length);
...
function buildDashboardData(reservations) {
  ...
  // ── All today's bookings (for reference) ──
  const todayBookings = reservations.filter((r) => r.isToday);

  // ── CONFIRMED only today (for stats) ──
  ...
  const thisMonthConfirmed = reservations.filter((r) => {
    ...
    return r.receivedAtDate >= monthStart;
  });

  console.log('═══════════════════════════════════════');
  console.log('BUILDING DASHBOARD');
  console.log('Today all:', todayBookings.length);
  console.log('Today confirmed:', todayConfirmed.length);
  console.log('This month confirmed:', thisMonthConfirmed.length);
  console.log('By brand (today confirmed):', {
    UNITED: todayConfirmed.filter(r => r.brand === 'UNITED').length,
    MOVIS: todayConfirmed.filter(r => r.brand === 'MOVIS').length,
    DRIVO: todayConfirmed.filter(r => r.brand === 'DRIVO').length,
  });
  console.log('═══════════════════════════════════════');
  ...
  BRANDS.forEach((brand) => {
    ...
    brandStats[brand] = {
      ...
      todayBookings: todayBrandConfirmed.length,  // ← CONFIRMED only
      yesterdayBookings: 0, // You can add yesterday logic if needed
    };
  });

  console.log('Brand stats:', brandStats);

  // ── Country stats: ALL TIME (for reference) ──
  const countryMap = {};
  ...
  todayConfirmed.forEach((r) => {  // ← Uses todayConfirmed, not todayBookings
    if (!r.country) return;
    const countryKey = r.country.trim();
  ...
  // ── Recent bookings (latest 20, all statuses for feed) ──
  ...
  return {
    meta: {
      ...
      totalThisMonth: thisMonthConfirmed.length,  // ← NEW: confirmed this month
    },
    todayStats: {
      totalBookings: todayConfirmed.length,  // ← CONFIRMED only
      dailyGoal,
      dailyProgress,
    },
    brandStats,
    countryStats,        // All-time (for reference)
    todayCountryStats,   // Today's confirmed (for dashboard)
    latestReservations,
    leaderboard,
    hourlyData,          // Today's confirmed only
  };
}
