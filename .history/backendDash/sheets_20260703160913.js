...
function parseDate(raw) {
  ...
  // Try 1: Direct new Date for ISO-like strings (YYYY-MM-DD HH:MM:SS)
  // This handles: "2026-07-03 14:23:00", "2026-07-03T14:23:00", etc.
  if (str.match(/^\d{4}-\d{2}-\d{2}/)) {
    const d = new Date(str);
    if (!isNaN(d.getTime())) {
      console.log('  ✓ Parsed as ISO-like:', d.toISOString());
      return d;
    }
  }
  
  // Try 2: DD/MM/YYYY HH:MM:SS (European format)
  ...
  // Try 3: DD/MM/YYYY HH:MM (no seconds)
  ...
  if (m) {
    ...
    if (!isNaN(d.getTime())) {
      console.log('  ✓ Parsed as DD/MM/YYYY HH:MM:', d.toISOString());
      return d;
    }
  }
  ...
  // Try 6: Month name format (Jul 3, 2026)
  ...
  // Try 7: European format DD.MM.YYYY HH:MM:SS
  ...
  // Try 8: American format M/D/YYYY HH:MM:SS
  m = str.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})[ T](\d{1,2}):(\d{2}):(\d{2})$/);
  if (m) {
...
async function fetchSheetData() {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: "A2:E10000",
      // NO valueRenderOption - use raw values
    });
