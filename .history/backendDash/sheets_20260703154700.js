...
function parseDate(raw) {
  ...
  // Try 3: DD/MM/YYYY HH:MM:SS or DD/MM/YYYY HH:MM (EUROPEAN FORMAT)
  // This MUST come before the American format to avoid confusion
  ...
  // Try 7: Month name format (Jul 3, 2026)
  ...
  // Try 8: European format DD.MM.YYYY HH:MM:SS
  m = str.match(/^(\d{1,2})\.(\d{1,2})\.(\d{4})[ T](\d{1,2}):(\d{2})(?::(\d{2}))?$/);
  if (m) {
    const [, day, month, year, hour, minute, second = '00'] = m;
    d = new Date(`${year}-${month.padStart(2,'0')}-${day.padStart(2,'0')}T${hour.padStart(2,'0')}:${minute}:${second}`);
  ...
  // Try 9: American format M/D/YYYY HH:MM:SS
  m = str.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})[ T](\d{1,2}):(\d{2})(?::(\d{2}))?$/);
  if (m) {
