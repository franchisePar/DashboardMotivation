...
function isToday(date) {
  ...
  // Use UTC to avoid timezone issues - compare YYYY-MM-DD strings
  const dateStr = date.toISOString().split('T')[0]; // "2026-07-03"
  const todayStr = new Date().toISOString().split('T')[0]; // "2026-07-03"
  const result = dateStr === todayStr;
  
  console.log('isToday:', date.toISOString(), '->', dateStr, '===', todayStr, '?', result);
  return result;
}
