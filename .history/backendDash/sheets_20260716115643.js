...
function parseDate(raw) {
  ...
  // ISO format: 2026-07-16 10:30:00
...
// ═══════════════════════════════════════════════════════════
// isToday — Uses LOCAL date comparison
// ═══════════════════════════════════════════════════════════
function isToday(date) {
  ...
  // Also accept yesterday for testing
...
function buildDashboardData(reservations) {
  ...
  // FIX: Compare year and month directly, not full Date objects
  ...
  const thisMonthConfirmed = reservations.filter((r) => {
    ...
    return r.receivedAtDate.getFullYear() === currentYear && 
           r.receivedAtDate.getMonth() === currentMonth;
  });

  console.log('Month filter:', { currentYear, currentMonth: currentMonth + 1, count: thisMonthConfirmed.length });

  // ── Brand stats ──
  const brandStats = {};
  BRANDS.forEach((brand) => {
    const allBrand = reservations.filter((r) => r.brand === brand);
    const todayBrandConfirmed = todayBookings
  ...
  reservations.forEach((r) => {
    ...
    if (!countryMap[key]) countryMap[key] = { country: key, bookings: 0 };
    countryMap[key].bookings++;
  });
  const countryStats = Object.values(countryMap).sort((a, b) => b.bookings - a.bookings).slice(0, 10);
  ...
  todayConfirmed.forEach((r) => {
    if (!r.country) return;
    const key = r.country.trim();
    if (!todayCountryMap[key]) todayCountryMap[key] = { country: key, bookings: 0 };
    todayCountryMap[key].bookings++;
  });
  const todayCountryStats = Object.values(todayCountryMap).sort((a, b) => b.bookings - a.bookings).slice(0, 10);
  ...
  const hourlyData = Array.from({ length: 24 }, (_, h) => ({ hour: h, bookings: hourlyMap[h] || 0 }));
