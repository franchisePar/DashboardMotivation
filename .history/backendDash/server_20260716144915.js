...
// ── State ───────────────────────────────────────────────
// CRITICAL: Start with NULL cache so first build creates fresh data
let cachedData = null;
let lastRowCount = 0;
...
io.on("connection", (socket) => {
  ...
  if (cachedData) {
    console.log(`Sending cached data to ${socket.id}, keys:`, Object.keys(cachedData));
...
server.listen(PORT, () => {
  ...
  console.log(`⚠️  Cache cleared on startup — fresh data will be built`);
});